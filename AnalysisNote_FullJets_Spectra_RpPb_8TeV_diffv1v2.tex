...
%DIF DEL AnalysisNote_FullJets_Spectra_RpPb_8TeV_v1.tex   Sun Jul 30 23:37:15 2023
%DIF ADD AnalysisNote_FullJets_Spectra_RpPb_8TeV_v2.tex   Mon Apr  1 16:49:16 2024
...
\hypersetup{
%DIF 22-28c22
%DIF <     pdfauthor={Austin Schmier},     
%DIF <     colorlinks=true,       
%DIF <     linkcolor=blue,          
%DIF <     citecolor=black,        
%DIF <     filecolor=magenta,      
%DIF <     urlcolor=blue           
%DIF < }
%DIF -------
    pdfauthor={Austin Schmier},     colorlinks=true,       linkcolor=blue,          citecolor=black,        filecolor=magenta,      urlcolor=blue           } %DIF > 
%DIF -------
...
%DIF 48-55c43-45
%DIF < \AtBeginDocument{
%DIF <   \renewcommand*{\AC@hyperlink}[2]{
%DIF <     \begingroup
%DIF <       \hypersetup{hidelinks}
%DIF <       \hyperlink{#1}{#2}
%DIF <     \endgroup
%DIF <   }
%DIF < }
%DIF -------
\AtBeginDocument{\renewcommand*{\AC@hyperlink}[2]{\begingroup %DIF > 
      \hypersetup{hidelinks}\hyperlink{#1}{#2}\endgroup %DIF > 
  }} %DIF > 
%DIF -------
\makeatother
...
%DIF 103a93-94
\newcommand{\RpPb}{\texorpdfstring{$R_{\text{\tiny pPb}}$}\xspace\xspace} %DIF > 
\newcommand{\RdAu}{\texorpdfstring{$R_{\text{\tiny dAu}}$}\xspace\xspace} %DIF > 
%DIF -------
...
\title{Inclusive Jet \pT Spectrum and Nuclear Modification Factor\\ in pp \DIFaddbegin \DIFadd{Collisions at }\s\DIFadd{~=~8~TeV }\DIFaddend and p--Pb Collisions at \sNN~=~\DIFdelbegin \DIFdel{8}\DIFdelend \DIFaddbegin \DIFadd{8.16}\DIFaddend ~TeV in ALICE}
...
\vspace{4cm}
\begin{abstract}
The differential cross-sections and nuclear modification factor are presented for fully reconstructed jets with R = 0.2 - \DIFdelbegin \DIFdel{0.6 }\DIFdelend \DIFaddbegin \DIFadd{0.5 }\DIFaddend in \pp collisions at \s~=~8~TeV and \DIFaddbegin \DIFadd{jets R = 0.2 - 0.4 in }\DIFaddend \pPb collisions at \sNN~=~8.16~TeV. Cross-section ratios are shown between R = 0.2 jets and the other radii. The nuclear modification factor, R$_{pPb}$, is defined as the ratio of the yield in \pPb to \pp collisions, scaled by the number of binary nucleon-nucleon collisions. Comparisons are made to \DIFdelbegin \DIFdel{a generator model using }\DIFdelend PYTHIA8 Monash \DIFaddbegin \DIFadd{and POWHEG+PYTHIA8}\DIFaddend . The \pp sample was collected with the ALICE detector in 2012, during run periods LHC12a - LHC12i. The \pPb sample was collected in 2016, during run periods LHC16r and LHC16s. The spectra analysis and subsequent analysis note sections were adapted from the methods used by Markus Fasel in his analysis at \s =~13~TeV \cite{anaNoteMFasel}.
\end{abstract}
...
The selected jet \pT range for \pPb collisions is \DIFaddbegin \DIFadd{as follows:
}
...
}\DIFaddend 20 - 240 GeV/c
    \DIFdelbegin \DIFdel{for all jet radii (}\DIFdelend \DIFaddbegin \item \DIFaddend R = \DIFdelbegin \DIFdel{0.2 }\DIFdelend \DIFaddbegin \DIFadd{0.3 \mbox{%DIFAUXCMD
$\rightarrow$
}%DIFAUXCMD
20 }\DIFaddend - \DIFdelbegin \DIFdel{0.5):
}\DIFdelend \DIFaddbegin \DIFadd{240 GeV/c
...
\DIFdelbegin \DIFdel{\textcolor{red}{Table is currently broken and causes pdflatex to crash. Will be fixed for next version.}
}%DIFDELCMD < 
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[hbt!]
    \DIFaddendFL \centering
    \DIFdelbeginFL %DIFDELCMD < \footnotesize
%DIFDELCMD <     \setlength\tabcolsep{5pt}
%DIFDELCMD <     \begin{tabular}{l rr r r} \toprule
%DIFDELCMD <         &\multicolumn{4}{c}{norm. Evt. $N_{\mbox{\tiny evt}}$} \\
%DIFDELCMD <         %%%
\DIFdelFL{Data set name }%DIFDELCMD < &   %%%
\DIFdelFL{INT7 }%DIFDELCMD < & %%%
\DIFdelFL{EMC7 }%DIFDELCMD < & %%%
\DIFdelFL{EJE/EJ1 }%DIFDELCMD < & %%%
\DIFdelFL{EJ2 }%DIFDELCMD < \\ \midrule
%DIFDELCMD <         %%%
\textbf{\DIFdelFL{LHC12a}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
$1.28 \cdot 10^7$
}%DIFAUXCMD
}%DIFDELCMD < \\ 
%DIFDELCMD <         %%%
\textbf{\DIFdelFL{LHC12b}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
$7.69 \cdot 10^6$
}%DIFAUXCMD
...
%DIFDELCMD <         %%%
\textbf{\DIFdelFL{LHC12c}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
$1.54 \cdot 10^7$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \caption{\DIFaddFL{Integrated luminosities for the }\pp \DIFaddFL{and }\pPb \DIFaddFL{datasets with downscale corrections.}}
    \begin{tabular}{  m{2.4cm}  m{3cm} m{3cm}  }
        \hline
        \DIFaddFL{System }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$1.19 \cdot 10^7$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Trigger Name }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$8.29 \cdot 10^5$
}%DIFAUXCMD
...
$\mathscr{L}_{\text{int}}  (\text{nb}^{-1})$
        ...
        \DIFdelbeginFL \textbf{\DIFdelFL{LHC12d}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
$2.99 \cdot 10^7$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \hline
        \pp \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$1.52 \cdot 10^7$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{INT7 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$1.10 \cdot 10^6$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.968 }\DIFaddendFL \\
            \DIFdelbeginFL \textbf{\DIFdelFL{LHC12f}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
$8.49 \cdot 10^6$
}%DIFAUXCMD
}\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$6.58 \cdot 10^6$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EMC7 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$5.12 \cdot 10^5$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{41.4 }\DIFaddendFL \\
            \DIFdelbeginFL \textbf{\DIFdelFL{LHC12h}}%DIFAUXCMD
\DIFdelFL{, pass2 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$2.45 \cdot 10^7$
}%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
...
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EJE }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$2.15 \cdot 10^6$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{588 }\DIFaddendFL \\ 
        \DIFdelbeginFL \textbf{\DIFdelFL{LHC12i}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
$2.33 \cdot 10^6$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \hline
        \pPb \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$6.61 \cdot 10^5$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{INT7 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$5.41 \cdot 10^5$
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{7.35\mbox{%DIFAUXCMD
$\times 10^{-3}$
}%DIFAUXCMD
}\DIFaddendFL \\
             \DIFdelbeginFL \textbf{\DIFdelFL{LHC16r}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\textcolor{red}{?} }%DIFDELCMD < & %%%
\DIFdelFL{n/a }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{\textcolor{red}{?} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EJ2 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\textcolor{red}{?} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.57\mbox{%DIFAUXCMD
$\times 10^{-2}$
}%DIFAUXCMD
}\DIFaddendFL \\
             \DIFdelbeginFL \textbf{\DIFdelFL{LHC16s}}%DIFAUXCMD
\DIFdelFL{, pass2 }%DIFDELCMD < & %%%
\DIFdelFL{\textcolor{red}{?} }%DIFDELCMD < & %%%
...
\DIFdelFL{Number of events per period for each trigger used.}}
    %DIFAUXCMD
%DIFDELCMD < \label{table:eventsPerPeriod}
%DIFDELCMD < %%%
...
\begin{figure}[h!]
    \centering
    \begin{multicols}{2}
            \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7.5cm]{figures/TriggerSwap/ratio_EMCINT_data.pdf}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=7.5cm]{figures/TriggerSwap/ratio_EMCINT_data_R02.pdf}
        \DIFaddendFL \vfill\null 
        \columnbreak
            \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7.5cm]{figures/TriggerSwap/ratio_EJEEMC_data.pdf}
%DIFDELCMD <         %%%
...
The \DIFdelbegin \DIFdel{rho sparse or "CMS" method of background subtraction is }\DIFdelend \DIFaddbegin \DIFadd{method }\DIFaddend used in this analysis to subtract the underlying event \DIFaddbegin \DIFadd{was developed by the CMS collaboration for sparse background environments and is termed the \mbox{%DIFAUXCMD
$\rho$
}%DIFAUXCMD
-sparse method~\mbox{%DIFAUXCMD
\cite{CMS:2012rmf}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . This method \DIFdelbegin \DIFdel{was also used in \mbox{%DIFAUXCMD
\cite{anaNoteHHassan}}\hspace{0pt}%DIFAUXCMD
, where more information about it can be found in section 4.1}\DIFdelend \DIFaddbegin \DIFadd{defines the transverse momentum background density as 
}

\begin{equation}
    \DIFadd{\rho_{\text{CMS}} = median\{\frac{{\text{\pT}}_{jet}^{k_T}}{A_{jet}^{k_T}}\} \cdot C
}\end{equation}
...
is the transverse momentum of a jet clustered with the \mbox{%DIFAUXCMD
$k$
}%DIFAUXCMD
\textsubscript{T} algorithm, \mbox{%DIFAUXCMD
$A_{jet}^{k_T}$
}%DIFAUXCMD
is the area of that jet, and \mbox{%DIFAUXCMD
$C$
}%DIFAUXCMD
is the particle occupancy factor, used to account for regions without any particles. It is defined as
}

\begin{equation}
    \DIFadd{C = \frac{\Sigma_{\text{j}}A_{\text{j}}}{A_{\text{acc}}}
}\end{equation}
...
is the area of each \mbox{%DIFAUXCMD
$k$
}%DIFAUXCMD
\textsubscript{T} jet with at least one real track and \mbox{%DIFAUXCMD
$A_{\text{acc}}$
}%DIFAUXCMD
is the total particle acceptance. It can be thought of as the covered area divided by the total area, showing how full or empty the event is. The \mbox{%DIFAUXCMD
$\rho$
}%DIFAUXCMD
distribution for all three triggers is shown in Figure~\ref{fig:Rho_distribution} for jets with resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, 0.3, and 0.4. The effect from background is greatest at low momenta and the effect decreases with increasing jet }\pT\DIFadd{. Since the INT7 trigger covers the low momentum range for this measurement, only the \mbox{%DIFAUXCMD
$\rho$
}%DIFAUXCMD
distribution for the INT7 trigger is used for this analysis. For \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, the distribution is smoothly falling with a maximum at zero. With increasing jet resolution parameter, the distribution becomes more peaked at zero and has a longer tail at high \mbox{%DIFAUXCMD
$\rho$
}%DIFAUXCMD
values. This is due to the fact that the highest momentum (signal) jet is excluded from the background calculation, and larger radius jets become increasing difficult to fit inside the acceptance without overlapping with the highest momentum jet. For this reason, and because the background distribution is calculated per unit area, the calculation of \mbox{%DIFAUXCMD
$\rho$
}%DIFAUXCMD
at a \mbox{%DIFAUXCMD
$k$
}%DIFAUXCMD
\textsubscript{T} jet resolution parameter of \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2 is used for all jet radii.
}

\DIFadd{A possible alternative to this method bypasses the limitation of the EMCal acceptance~\mbox{%DIFAUXCMD
\cite{anaNoteMConnors}}\hspace{0pt}%DIFAUXCMD
. Instead of calculating the background from a cone within the EMCal acceptance, the entire TPC acceptance is used to calculate the background. A scaling factor is then applied to \mbox{%DIFAUXCMD
$\rho$
}%DIFAUXCMD
, defined as 
}
...
\begin{equation}
    \DIFadd{S = \frac{E_{\text{EMC}}+\text{\pT}_\text{EMC}}{\text{\pT}_\text{TPC}}\frac{A_{\text{TPC}}}{A_{\text{EMC}}}
}\end{equation}

\noindent
\DIFadd{The first term is the ratio of the sum of the total energy from clusters in the EMCal after the hadronic correction is applied and the total momentum of all charged track pointing towards the EMCal acceptance to the total momentum of all charged tracks within the TPC acceptance. The second term is a normalization factor for the relative acceptances of the TPC and EMCal. As an extension of this thesis, this background subtraction technique will be applied to the }\pPb \DIFadd{data in order to compare these methods.
}
...
\begin{figure}[hbt!]
  ...
  \caption{\DIFaddFL{Rho distribution for the INT7, EJ2, and EJ1 triggers in }\pPb \DIFaddFL{collisions for jets with resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, 0.3, and 0.4.}}
...
\DIFadd{The corrected jet momentum is then calculated as 
}

\begin{equation}
    \DIFadd{\text{\pT}_{,jet}^{\text{corr}} = \text{\pT}_{,jet}^{\text{raw}} - \rho_{\text{CMS}} \cdot A_{\text{jet}}.
}\end{equation}

\noindent
\DIFadd{The background is calculated on an event-by-event basis using only particles within the jet acceptance and assumes a uniform background density. The true background density for each jet fluctuates around this average background density. This effect of background fluctuations is accounted for during the unfolding procedure by first using the random cones approach~\mbox{%DIFAUXCMD
\cite{ALICE:2012nbx} }\hspace{0pt}%DIFAUXCMD
to find the \mbox{%DIFAUXCMD
$\delta$
}%DIFAUXCMD
-}\pT \DIFadd{distribution. A cone with resolution parameter equivalent to the jet resolution parameter is randomly placed inside the \mbox{%DIFAUXCMD
$\eta-\phi$
}%DIFAUXCMD
acceptance of each event. The cone cannot overlap with the highest momentum jet and must be contained within the jet acceptance. The background fluctuations are then defined as 
}

\begin{equation}
    \DIFadd{\delta \text{\pT} = \text{\pT}_\text{,RC} - \rho_{\text{CMS}} \pi R_{\text{cone}}^2
}\end{equation}
...
is the }\pT \DIFadd{of the random cone and \mbox{%DIFAUXCMD
$R_{\text{cone}}$
}%DIFAUXCMD
is its resolution parameter. The \mbox{%DIFAUXCMD
$\delta$
}%DIFAUXCMD
-}\pT \DIFadd{distribution for all three triggers is shown in Figure~\ref{fig:DeltaPt_distribution} for jets with resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, 0.3, and 0.4. The distribution for INT7 is used since this is where the background dominates. The distribution is centered at zero with a steep tail toward negative values caused by fluctuations from soft processes. The longer tail toward positive values originates from both background fluctuations and signal}\DIFaddend .
...
-}\pT \DIFaddFL{distribution for the INT7, EJ2, and EJ1 triggers in }\pPb \DIFaddFL{collisions for jets with resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, 0.3, and 0.4.}}
...
\begin{figure}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \begin{multicols}{2}
%DIFDELCMD <             \includegraphics[width=7.5cm]{figures/CorrRawSpec/corrRawSpec_INT7.pdf}
%DIFDELCMD <             \includegraphics[width=7.5cm]{figures/CorrRawSpec/corrRawSpec_EMC7.pdf}
%DIFDELCMD <         \vfill\null 
%DIFDELCMD <         \columnbreak
%DIFDELCMD <             \includegraphics[width=7.5cm]{figures/CorrRawSpec/corrRawSpec_EJE.pdf}
%DIFDELCMD <         \vfill\null
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=15cm]{figures/CorrRawSpec/corrRawSpec_Combined.pdf}
%DIFDELCMD <     \includegraphics[width=15cm]{figures/pPbFigures/CorrRawSpec/corrRawSpec_Combined.pdf}
%DIFDELCMD <     \vfill\null
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Combined raw spectrum after corrections for the rejection factors, vertex finding efficiency, trigger efficiency, and cross-section normalization for }%DIFDELCMD < \pp %%%
\DIFdelFL{(top) and }%DIFDELCMD < \pPb %%%
\DIFdelFL{(bottom).}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:CombinedRawSpec}
...
%DIFDELCMD < \label{sec:nuclearModFac}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textcolor{red}{Section on building the RpA.}}%DIFDELCMD < \clearpage{}
%DIFDELCMD < \clearpage{}%%%
...
\DIFdelbegin \DIFdel{\textcolor{red}{RpA results}
}\DIFdelend \DIFaddbegin \DIFadd{The nuclear modification factor is found by taking the ratio of the jet cross-section in }\pPb \DIFadd{to that in }\pp \DIFadd{scaled by 208. Alternate, equivalent methods for calculating the }\RpPb \DIFadd{are 
}

\begin{equation}
    \DIFadd{\text{\RpPb}(\text{\pT}) = \frac{1}{208} \frac{ d\sigma^{\text{pPb}}/d\text{\pT} }{ d\sigma^{\text{\pp}}/d\text{\pT} } = \frac{1}{T_\text{pPb}} \frac{ dN^{\text{pPb}}/d\text{\pT} }{ d\sigma^{\text{\pp}}/d\text{\pT} } = \frac{1}{\langle N_\text{coll}\rangle} \frac{ dN^{\text{pPb}}/d\text{\pT} }{ dN^{\text{\pp}}/d\text{\pT} }
...
are the cross-sections for }\pp \DIFadd{and }\pPb\DIFadd{, \mbox{%DIFAUXCMD
$N$
}%DIFAUXCMD
are the yields, \mbox{%DIFAUXCMD
$T_{\text{pPb}}$
}%DIFAUXCMD
is the nuclear overlap function, and \mbox{%DIFAUXCMD
$\langle N_\text{coll}\rangle$
}%DIFAUXCMD
is the average number of binary nucleon-nucleon collisions. The latter two values are extracted from Glauber models. Because the center of mass energy of }\pPb \DIFadd{collisions for this measurement is 8.16 TeV while the center of mass energy for }\pp \DIFadd{collisions is 8 TeV, the }\pp \DIFadd{spectrum must be scaled from 8 TeV to 8.16 TeV to be consistent with the }\pPb \DIFadd{spectrum. This is performed by taking the ratio of generated PYTHIA jets in 8.16 and 8 TeV and applying this as a momentum-dependent scaling factor to the }\pp \DIFadd{spectrum, effectively giving the }\pp \DIFadd{spectrum at 8.16 TeV. By using PYTHIA to scale the spectrum, a model dependence is introduced into the measurement, although small. The small rapidity shift introduced in }\pPb \DIFadd{collisions is not accounted for in the generated PYTHIA events. The scaling factor is on the order of approximately 3\%, which is comparable to the level of the statistical uncertainty on the spectrum and far below the systematic uncertainty. For \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, the scaling factor is shown in Figure~\ref{fig:PythiaScaleFactor}. The scale factors for the remaining radii are nearly identical and can be found in Appendix~\ref{sec:appendixScaleFactors}.
}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=\textwidth]{figures/ScaleFactorPythia/PythiaRatio_R02.png}
    \caption{\DIFaddFL{Ratio of generated }\pp \DIFaddFL{PYTHIA events at 8.16 TeV to 8 TeV for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2 jets.}}
...
\DIFadd{The nuclear modification factor for R = 0.2 to 0.4 can be found in Figure~\ref{fig:finalRpPb}. For R = 0.4 and R = 0.3, the }\RpPb \DIFadd{is consistent with unity within uncertainties. With R = 0.2 jets, the ratio begins to deviate from 1 at high-}\pT\DIFadd{, suggesting a possible presence of jet modification for jets with small resolution parameters.
}
...
\begin{figure}[hbt!]
    ...
    \caption{\DIFaddFL{Nuclear modification factor (}\RpPb\DIFaddFL{) for R = 0.2 (top left), R = 0.3 (top right), and R = 0.4 (bottom).}}
    \label{fig:finalRpPb}
\end{figure}

\DIFadd{This is predominantly consistent with previous measurements performed by ALICE, ATLAS, CMS, and PHENIX, shown in Figure~\ref{fig:RpPbComp}; although, the CMS measurement shows a slight enhancement around 100 GeV/c. Since charged particle jets only reconstruct the charged component of the jet, the yields will be shifted down in jet }\pT \DIFadd{with respect to the equivalent fully reconstructed jet population. The charged jet }\RpPb \DIFadd{is scaled by 1.5 to approximately account for the difference in jet reconstruction between charged and fully reconstructed jets. Due to higher statistics from the use of EMCal triggers, the uncertainties on fully reconstructed jets are smaller than those on charged particle jets with the exception of the }\pT \DIFadd{range in the trigger transition region. ALICE measurements of the }\RpPb \DIFadd{in charged and fully reconstructed jets are in agreement for all resolution parameters.
}

\DIFadd{The }\RpPb \DIFadd{for resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2 falls slightly below one at high-}\pT\DIFadd{, suggesting possible suppression for jets with a small resolution parameter at high-}\pT\DIFadd{. The previous full jet measurements that extend to this kinematic range did not measure jets at resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2, so no claims can be made about agreement.
}
...
\begin{figure}[hbt!]
    ...
    \caption{\DIFaddFL{Comparison of the jet nuclear modification factors }\RpPb \DIFaddFL{(ALICE, ATLAS, CMS) and }\RdAu \DIFaddFL{(PHENIX) for R = 0.2 (top left), R = 0.3 (top right), and R = 0.4 (bottom). The }\pT \DIFaddFL{for charged particle jets in ALICE is scaled by 1.5.}}
    \label{fig:RpPbComp}
\end{figure}

\DIFadd{Although previous measurements using larger radius fully reconstructed jets showed little to no modification, the work presented here shows a slight suppression of \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2 jets at high-}\pT\DIFadd{. This observation suggests that the transverse momentum is more spread out in \mbox{%DIFAUXCMD
$\eta-\phi$
}%DIFAUXCMD
space in }\pPb \DIFadd{collisions, and while jets with resolution parameter \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.4 or greater recapture the full momentum, small radius jets are sensitive to this modification.
}\DIFaddend 

\subsection{Comparison to \DIFdelbegin \DIFdel{PYTHIA8}\DIFdelend \DIFaddbegin \DIFadd{PYTHIA and POWHEG}\DIFaddend }
\label{sec:mcComparison}

The jet spectra \DIFaddbegin \DIFadd{and ratios }\DIFaddend in \pp \DIFaddbegin \DIFadd{collisions }\DIFaddend are compared to calculations \DIFdelbegin \DIFdel{done in the framework of }\DIFdelend \DIFaddbegin \DIFadd{using PYTHIA8 Monash and POWHEG+}\DIFaddend PYTHIA8. Fig. \ref{fig:MCGen} shows the comparison of the \DIFaddbegin \pp \DIFaddend jet spectra for jet radii R = 0.2 and R = \DIFdelbegin \DIFdel{0.6 }\DIFdelend \DIFaddbegin \DIFadd{0.5 }\DIFaddend to the generator \DIFdelbegin \DIFdel{model}\DIFdelend \DIFaddbegin \DIFadd{models}\DIFaddend , and \ref{fig:MCGen_RatioDataMC} shows the ratio of simulation to \DIFaddbegin \pp \DIFaddend ALICE data. Fig. \ref{fig:MCGen_Ratio} shows the ratios of radii 0.2/0.3 and 0.2/\DIFdelbegin \DIFdel{0.6 }\DIFdelend \DIFaddbegin \DIFadd{0.5 }\DIFaddend compared to the generator \DIFdelbegin \DIFdel{model}\DIFdelend \DIFaddbegin \DIFadd{models for }\pp \DIFadd{collisions}\DIFaddend . It is known that PYTHIA overshoots the \pp data\DIFdelbegin \DIFdel{by roughly 40-50\%}\DIFdelend . It has been seen in all collision systems so far, and can also be observed here. \DIFaddbegin \DIFadd{POWHEG also overestimates the data, although to a lesser degree.
}\DIFaddend 
...
\begin{figure}
    ...
    \begin{multicols}{2}
        ...
        \vfill\null
        \columnbreak
            \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7.5cm]{figures/MCGen/MCComp_R06_nooutlier.pdf}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=7.5cm]{figures/MCGen/MCComp_R05_nooutlier.pdf}
        \DIFaddendFL \vfill\null
    \end{multicols}
    \caption{Comparison to PYTHIA8 \DIFaddbeginFL \DIFaddFL{Monash and POWHEG+PYTHIA8 }\DIFaddendFL for jet radii 0.2 and \DIFdelbeginFL \DIFdelFL{0.6}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.5 in }\pp \DIFaddFL{collisions}\DIFaddendFL .}
    \label{fig:MCGen}
\end{figure}
...
\begin{figure}
    ...
    \begin{multicols}{2}
        ...
        \vfill\null
        \columnbreak
            \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7.5cm]{figures/MCGen/MCComp_Ratio_R0602_nooutlier.pdf}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=7.5cm]{figures/MCGen/MCComp_Ratio_R0502_nooutlier.pdf}
        \DIFaddendFL \vfill\null
    \end{multicols}
    \caption{Comparison to PYTHIA8 \DIFaddbeginFL \DIFaddFL{Monash and POWHEG+PYTHIA8 }\DIFaddendFL for ratios of jet radii 0.2/0.3 and 0.2/\DIFdelbeginFL \DIFdelFL{0.6}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.5 in }\pp \DIFaddFL{collisions}\DIFaddendFL .}
...
\begin{figure}
    ...
    \begin{multicols}{2}
        ...
        \vfill\null
        \columnbreak
            \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=7.5cm]{figures/MCGen/ratioDataMC/ratio_simdata_R06_nooutlier.pdf}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=7.5cm]{figures/MCGen/ratioDataMC/ratio_simdata_R05_nooutlier.pdf}
        \DIFaddendFL \vfill\null
    \end{multicols}
    \caption{Ratio of PYTHIA8 \DIFdelbeginFL \DIFdelFL{simulation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Monash and POWHEG+PYTHIA8 }\DIFaddendFL to ALICE data for jet radii 0.2 and \DIFdelbeginFL \DIFdelFL{0.6}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.5 in }\pp \DIFaddFL{collisions}\DIFaddendFL .}
    \label{fig:MCGen_RatioDataMC}
\end{figure}
\DIFaddbegin 

\DIFadd{Additionally, the jet spectra and ratios in }\pPb \DIFadd{collisions are compared to calculations using PYTHIA8 Monash and POWHEG+PYTHIA8. PYTHIA8 simulates }\pp \DIFadd{collisions, and a nuclear PDF was not used for the POWHEG production. Thus, both simulate only }\pp \DIFadd{collisions. In addition, PYTHIA does not take into account the rapidity shift that is present in data. Fig. \ref{fig:MCGen_pPb} shows the comparison of the }\pPb \DIFadd{jet spectra for jet radii R = 0.2 and R = 0.5 to the generator models, and \ref{fig:MCGen_RatioDataMC_pPb} shows the ratio of simulation to }\pPb \DIFadd{ALICE data. Fig. \ref{fig:MCGen_Ratio_pPb} shows the ratios of radii 0.2/0.3 and 0.2/0.5 compared to the generator models for }\pPb \DIFadd{collisions. 
}
...
\begin{figure}
    ...
    \begin{multicols}{2}
        ...
        \vfill\null
    \end{multicols}
    \caption{\DIFaddFL{Comparison to PYTHIA8 Monash and POWHEG+PYTHIA8 for jet radii 0.2 and 0.4 in }\pPb \DIFaddFL{collisions.}}
    \label{fig:MCGen_pPb}
\end{figure}
...
\begin{figure}
    ...
    \begin{multicols}{2}
        ...
        \vfill\null
    \end{multicols}
    \caption{\DIFaddFL{Comparison to PYTHIA8 Monash and POWHEG+PYTHIA8 for ratios of jet radii 0.2/0.3 and 0.2/0.4 in }\pPb \DIFaddFL{collisions.}}
    \label{fig:MCGen_Ratio_pPb}
\end{figure}

\section{\DIFadd{Collision Energy Dependence in }\pp \DIFadd{Collisions}}
\label{sec:CollEnergyDep}

\DIFadd{Figure~\ref{fig:specCompareR04} shows the jet cross-sections in }\pp \DIFadd{collisions at }\s \DIFadd{= 2.76, 5.02, 8, and 13 TeV for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.4. With increasing collision energy, it becomes more likely to create a high-momentum jet. This behavior is also seen in Figure~\ref{fig:sqrtSCompareR04}, which shows the collision energy dependence of the jet cross-section in }\pp \DIFadd{collisions for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.4 in various ranges of jet }\pT\DIFadd{. The spectra are fit with a power law in each }\pT \DIFadd{range. An increasing power law exponent is observed for increasing jet }\pT\DIFadd{.
}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=\textwidth]{figures/EnergyComparisons/SpectrumComparison_R04.pdf}
    \caption{\DIFaddFL{Jet cross-section measurements in }\pp \DIFaddFL{collisions at }\s \DIFaddFL{= 2.76, 5.02, 8, and 13 TeV for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.4.}}
...
\begin{figure}[hbt!]
    ...
    \caption{\DIFaddFL{Collision energy dependence of the jet cross-section in }\pp \DIFaddFL{collisions for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.4 in various ranges of jet }\pT\DIFaddFL{, fit with a power law in each }\pT \DIFaddFL{range.}}
    \label{fig:sqrtSCompareR04}
\end{figure}

\DIFadd{Figure~\ref{fig:ratioCompareR03} shows the ratios of the jet cross-sections for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2/0.3 for }\pp \DIFadd{collisions at }\s \DIFadd{= 5.02, 8, and 13 TeV. The ratio is consistent between all collision energies within uncertainties, indicating that jet fragmentation pattern is similar between the different collision energies.
}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=\textwidth]{figures/EnergyComparisons/RatioComparison_R03.pdf}
    \caption{\DIFaddFL{Ratio of jet cross-section measurements for \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
= 0.2/0.3 in }\pp \DIFaddFL{collisions at }\s \DIFaddFL{= 5.02, 8, and 13 TeV.}}
    \label{fig:ratioCompareR03}
\end{figure}

\DIFadd{Collision energy dependence plots for the cross-section and ratios for the remaining jet resolution parameters can be found in Appendix~\ref{sec:appendixCollEnergyDep}.}\DIFaddend \clearpage{}
...
\section{\DIFadd{Generated PYTHIA8 Scale Factors}}\label{sec:appendixScaleFactors}
...
\section{\DIFadd{Collision Energy Dependence for All Jet Radii}}\label{sec:appendixCollEnergyDep}
...
= 0.2 in various ranges of jet }\pT\DIFaddFL{, fit with a power law in each }\pT \DIFaddFL{range.}}
...
= 0.3 in various ranges of jet }\pT\DIFaddFL{, fit with a power law in each }\pT \DIFaddFL{range.}}
...
= 0.5 in various ranges of jet }\pT\DIFaddFL{, fit with a power law in each }\pT \DIFaddFL{range.}}
...
= 0.2/0.4 in }\pp \DIFaddFL{collisions at }\s \DIFaddFL{= 5.02, 8, and 13 TeV.}}
...
= 0.2/0.5 in }\pp \DIFaddFL{collisions at }\s \DIFaddFL{= 5.02, 8, and 13 TeV.}}
    \label{fig:appRatioCompareR05}
\end{figure}
\fi\DIFaddend \clearpage{}
